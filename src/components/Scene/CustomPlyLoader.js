...
!!! This is a modified version of THREE.PLYLoader !!!
All custom properties are loaded as attributes by default.
I needed this to load a PLY file with deviation values for each vertex.
 ...
 * } );
 *
 * Custom properties outside of the defaults (position, uv, normal and color)
 * are loaded by default as attributes with the same name.
 *
 */

const _color = new Color();

class CustomPLYLoader extends Loader {
  constructor(manager) {
    super(manager);
  ...
  parse(data) {
    ...
    function mapElementAttributes(properties, buffer) {
      ...
      function findStdAttr(names) {
        for (let i = 0, l = names.length; i < l; i++) {
          const name = names[i];

          if (elementNames.includes(name)) {
            standardAttributes.add(name); // !!!
            return name;
          }
        }
      ...
      const allAttributes = {
        attrX: findStdAttr(['x', 'px', 'posx']) || 'x',
        attrY: findStdAttr(['y', 'py', 'posy']) || 'y',
        attrZ: findStdAttr(['z', 'pz', 'posz']) || 'z',
        attrNX: findStdAttr(['nx', 'normalx']),
        attrNY: findStdAttr(['ny', 'normaly']),
        attrNZ: findStdAttr(['nz', 'normalz']),
        attrS: findStdAttr(['s', 'u', 'texture_u', 'tx']),
        attrT: findStdAttr(['t', 'v', 'texture_v', 'ty']),
        attrR: findStdAttr(['red', 'diffuse_red', 'r', 'diffuse_r']),
        attrG: findStdAttr(['green', 'diffuse_green', 'g', 'diffuse_g']),
        attrB: findStdAttr(['blue', 'diffuse_blue', 'b', 'diffuse_b']),
      };

      // !!! This is the main change to the original code !!!
      const customAttributes = elementNames.filter((name) => !standardAttributes.has(name));
      for (const customAttribute of customAttributes) {
        scope.customPropertyMapping[customAttribute] = [customAttribute];
        if (!(customAttribute in buffer)) {
          buffer[customAttribute] = [];
        }
      }
      // !!! End !!!
    ...
    function parseASCII(data, header) {
      ...
      loop: for (let i = 0; i < header.elements.length; i++) {
        ...
        const attributeMap = mapElementAttributes(elementDesc.properties, buffer);
    ...
    function parseBinary(data, header) {
      ...
      for (let currentElement = 0; currentElement < header.elements.length; currentElement++) {
        ...
        const attributeMap = mapElementAttributes(properties, buffer);
